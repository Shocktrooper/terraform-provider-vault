...
func TestAccSSHSecretBackendRole_defaultExtensionsTemplate(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			testutil.GetImportTestStep(resourceName, false, nil, consts.FieldAllowEmptyPrincipals, consts.FieldPort),
		},
	})
}
...
func TestAccSSHSecretBackendRole_extensionsTemplateAndOtpFieldsInvalid(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				// Test: Invalid port number (too high) - Remove ExpectError to see what Vault accepts
			...
			{
				// Test: Invalid port number (negative) - Remove ExpectError to see what Vault accepts
